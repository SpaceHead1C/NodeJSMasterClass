...
var dns = require('dns');
var _perfomance = require('perf_hooks').performance;
var util = require('util');
var debug = util.debuglog('perfomance');
...
// Users – put
// Required data: phone
// Optional data: firstName, lastName, password (at last one must be specified)
...
handlers._tokens.post = function(data, callback) {
    ...
    if (phone && password) {
        ...
        _perfomance.mark('beginning user lookup');
        _data.read('users', phone, function(err, userData) {
            _perfomance.mark('user lookup complete');
            if (!err && userData) {
                // Hash the sent password and compare it to the password stored in the user object
                _perfomance.mark('beginning password hashing');
                var hashedPassword = helpers.hash(password);
                ...
                if (hashedPassword == userData.hashedPassword) {
                    ...
                    // Store the token
                    _perfomance.mark('beginning storing token');
                    _data.create('tokens', tokenId, tokenObject, function(err) {
                        _perfomance.mark('storing token complete');

                        //Gethet all the measurements
                        _perfomance.measure('Beginning to end', 'entered function', 'storing token complete');
                        _perfomance.measure('Validating user input', 'entered function', 'inputs validated');
                        _perfomance.measure('User lookup', 'beginning user lookup', 'user lookup complete');
                        _perfomance.measure('Password hashing', 'beginning password hashing', 'password hashing complete');
                        _perfomance.measure('Token data creation', 'creating data for token', 'beginning storing token');
                        _perfomance.measure('Token storing', 'beginning storing token', 'storing token complete');

                        // Log out all the measurements
                        var measurements = _perfomance.getEntriesByType('measure');
        ...
        _data.read('tokens', id, function(err, tokenData) {
            if (!err && tokenData) {
                ...
                if (tokenData.expires > Date.now()) {
                    // Set tje expiration an hour from now
                    ...
                    if (tokenIsValid) {
                        ...
                        _data.delete('checks', id, function(err) {
                            if (!err) {
                                ...
                                callback(500, {'Error': 'Coulde not delete check data'});
                            }
                        });
                    } else {
                        callback(403);
